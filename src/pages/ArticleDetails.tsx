...
export default function ArticleDetails() {
  ...
  return (
    <>
      ...
      <div className="my-12 p-8 bg-gradient-to-r from-yellow-500/10 to-transparent border border-yellow-500/20 rounded-[2rem] text-center max-w-2xl mx-auto flex flex-col items-center">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">هل وجدت هذا المقال مفيدًا؟</h3>
        <p className="text-gray-600 dark:text-gray-400 mb-6 text-sm">شارك المعرفة مع أصدقائك في مجتمع الكريبتو!</p>
        <div className="flex items-center gap-4 justify-center">
          <a 
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title + " - مقال رائع من كريبتو بالعربي! 🚀")}&url=${encodeURIComponent(shareUrl)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-5 py-2.5 bg-[#1DA1F2] hover:bg-[#1a91da] text-white rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-1"
          >
            <Twitter className="w-5 h-5" />
            غرد بها
          </a>
          <a 
            href={`https://api.whatsapp.com/send?text=${encodeURIComponent("مقال مهم عن الكريبتو 🚀: " + article.title + ' ' + shareUrl)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-5 py-2.5 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-1"
          >
